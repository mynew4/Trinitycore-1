...
bool bot_ai::doCast(Unit* victim, uint32 spellId, bool triggered, uint64 originalCaster)
{
    ...
    if (!victim->IsWithinLOSInMap(me) && IsInBotParty(victim))
    {
        ...
        //me->MonsterWhisper(msg.str(), master->GetGUID());
        me->Relocate(victim);
    }
    ...
    for (GroupReference* itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (doCast(target, REZZ))//rezzing it
        {
            ...
            if (tPlayer != master)
            {
                ...
                me->MonsterWhisper("Rezzing ", master->GetGUID());
            }
            rezz_cd = 60;
            return;
        }
...
void bot_ai::listAuras(Player* player, Unit* unit) const
{
    ...
    ch.PSendSysMessage("ListAuras for %s, %s", unit->GetName(), botstring.str());
    ...
    for (uint8 i = STAT_STRENGTH; i != MAX_STATS; ++i)
    {
        ...
        ch.PSendSysMessage("%s: %f", mystat, unit->GetTotalStatValue(Stats(i)));
    }
...
void bot_minion_ai::setStats(uint8 myclass, uint8 myrace, uint8 mylevel, bool force)
{
    ...
    if (mylevel >= 40 && spp_mod != 0.f)
    {
        ...
        for (uint8 i = SPELL_SCHOOL_HOLY; i != MAX_SPELL_SCHOOL; ++i)
        {
            int32 power = master->SpellBaseDamageBonus(SpellSchoolMask(1 << i));
...
void bot_pet_ai::setStats(uint8 mylevel, uint8 petType, bool force)
{
    ...
    //sLog->outError("setStats(): Updating pet bot %s, type: %u, level %u, owner: %s, master: %s", me->GetName(), petType, mylevel, m_creatureOwner->GetName(), master->GetName());
...
bool bot_ai::RefreshAura(uint32 spell, int8 count, Unit* target) const
{
    ...
    //if (!spellInfo->IsPassive())
    //{
    //    sLog->outError("bot_ai::RefreshAura(): %s received spell %u (%s) which is not a passive spell!", target->GetName(), spell, spellInfo->SpellName[0]);
    //    //return false;
    //}
...
Unit* bot_ai::getTarget(bool byspell, bool ranged, bool &reset) const
{
    ...
    if (u && u == mytar)
    {
        //sLog->outError("bot %s continues attack common target %s", me->GetName(), u->GetName());
        return u;//forced
    }
    //Follow if...
    uint8 followdist = master->GetBotFollowDist();
    ...
    {
        //sLog->outError("bot %s starts attack master's target %s", me->GetName(), u->GetName());
        return u;
    }

    if (CanBotAttack(mytar, byspell) && !InDuel(mytar))
    {
        //sLog->outError("bot %s continues attack its target %s", me->GetName(), mytar->GetName());
    ...
    if (!gr)
    {
        for (uint8 i = 0; i != master->GetMaxNpcBots(); ++i)
        {
            ...
            {
                //sLog->outError("bot %s hooked %s's victim %s", me->GetName(), bot->GetName(), u->GetName());
                return u;
            }
            ...
            if (u && CanBotAttack(u, byspell) && 
                ...
                (master->isDead() || master->GetDistance(u) < foldist))
            {
                //sLog->outError("bot %s hooked %s's victim %s", me->GetName(), pet->GetName(), u->GetName());
                return u;
            }
        }
    }
    else
    {
        for (GroupReference* ref = gr->GetFirstMember(); ref != NULL; ref = ref->next())
        {
            ...
            if (u && pl != master && CanBotAttack(u, byspell) && 
                ...
                (master->isDead() || master->GetDistance(u) < foldist))
            {
                //sLog->outError("bot %s hooked %s's victim %s", me->GetName(), pl->GetName(), u->GetName());
                return u;
            }
    ...
    {
        //sLog->outError("bot %s has Found new target %s", me->GetName(), t->GetName());
        reset = true;
    }
    return t;
}
...
{
    //sLog->outError("_OnHealthUpdate(): updating bot %s", me->GetName());
    ...
    stammod -= 0.3f;
    //sLog->outError("stammod: %f", stammod);
    ...
    //sLog->outError("stat to health add: Stamina (%f), value: %f", stamValue, stamValue*stammod);
    ...
    //sLog->outError("health to remove from gender mod: %i", -miscVal);
    hp_add -= miscVal;//less hp for females lol
    //sLog->outError("health to add after gender mod: %i", hp_add);
    //miscVal = myrace*(mylevel/5);
    //sLog->outError("health to add from race mod: %i", miscVal);
    //hp_add += miscVal;//draenei tanks lol
    //sLog->outError("health to add after race mod: %i", hp_add);
    miscVal = master->GetNpcBotSlot(me->GetGUID()) * (mylevel/5);
    //sLog->outError("health to remove from slot mod: %i", -miscVal);
    hp_add -= miscVal;
    //sLog->outError("health to add after slot mod: %i", hp_add);
...
void bot_minion_ai::_OnManaUpdate(uint8 myclass, uint8 mylevel) const
{
    if (me->getPowerType() != POWER_MANA)
        return;
    //sLog->outError("_OnManaUpdate(): updating bot %s", me->GetName());
    ...
    //sLog->outError("Manamod: %f", manamod);
    ...
    //sLog->outError("bot's stats to mana add: Int (%f), value: %f", intValue, intValue * manamod);
    ...
    if (master->getPowerType() == POWER_MANA)
    {
        ...
        intValue = stat * 0.5f;
    //sLog->outError("mana add from master's stat: %f", intValue * manamod);
    m_basemana += intValue * manamod;
    //sLog->outError("base mana + mana from master's intellect or stat: %f", m_basemana);
    ...
    //sLog->outError("mana to add from gender mod: %u", otherVal);
    m_basemana += float(otherVal);//more mana for females lol
    //sLog->outError("base mana after gender mod: %f", m_basemana);
    otherVal = master->GetNpcBotSlot(me->GetGUID()) * (mylevel/5);// only to make mana unique
    //sLog->outError("mana to remove from slot mod: %i", -int8(otherVal));
    m_basemana -= otherVal;
    //sLog->outError("base mana after slot mod: %f", m_basemana);
    float m_totalmana = m_basemana;
    //sLog->outError("total mana to set: %f", m_totalmana);
    me->SetModifierValue(UNIT_MOD_MANA, BASE_VALUE, m_totalmana);
    me->UpdateMaxPower(POWER_MANA);
    //sLog->outError("Overall mana to set: %u", me->GetMaxPower(POWER_MANA));
        ...
        for (uint8 i = 0; i != master->GetMaxNpcBots(); ++i)
        {
            ...
            if (!mgr.isEmpty())
            {
                ...
                for (std::set<Unit*>::const_iterator i = Set.begin(); i != Set.end(); ++i)
                {
                    ...
                    if (/*unit->IsFriendlyTo(master)*/IsInBotParty(unit) || !unit->isInCombat())
                    {
                        //ch.PSendSysMessage("_OnEvade(): %s's hostile reference is removed from %s!", unit->GetName(), m_botsPet->GetName());
                        mgr.deleteReference(unit);
                    }
                }
            }
            ...
            //    {
            //        //ch.PSendSysMessage("_OnEvade(): %s's HostileRef is empty! Combatstop!", pet->GetName());
            ...
            switch (sender)
            {
                case 6001:// food/drink
                {
                    ...
                    if (!food)
                    {
                        ...
                        errorstr += " yet";
                        creature->MonsterWhisper("I can't conjure ", player->GetGUID());
