...
public:
    ...
    bool revive(Player& botPlayer) { return HandleReviveCommand(this, botPlayer.GetName()); }
    bool teleport(Player& botPlayer) { return HandleSummonCommand(this, botPlayer.GetName()); }
...
private:
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
            ...
            //    ChatHandler(target).PSendSysMessage(LANG_SUMMONED_BY, handler->playerLink(_player->GetName()));
...
void PlayerbotAI::UpdateDeadActions(uint32 const diff)
{
    ...
    if (me->getDeathState() == CORPSE)
    {
        ...
        //if (me->GetCorpse())
        //{
        //    sLog->outError("PlayerbotAI: UpdateDeadActions - %s already has a corpse!", me->GetName());
    ...
    else if (me->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GHOST))
    {
        ...
        if (!corpse)
        {
            //sLog->outError("PlayerbotAI: UpdateAI - %s has no corpse!", me->GetName());
        ...
        {
            //sLog->outError("PlayerbotAI: UpdateAI - Teleport %s to corpse...", me->GetName());
        ...
        if (delay > 0)
        {
            //sLog->outError("PlayerbotAI: UpdateAI - %s has to wait for %u seconds to revive...", me->GetName(), uint32(delay));
            return;
        }
        //revive by master
        //sLog->outError("PlayerbotAI: UpdateAI - Reviving %s...", me->GetName());
        if (!PlayerbotChatHandler(GetMaster()->GetSession()).revive(*me))
        {
            //sLog->outError("PlayerbotAI: %s could not be revived ...", me->GetName());
            return;
        }
    }
    _canSelfRes = false;
...
void PlayerbotAI::UpdateFollowActions(uint32 const diff)
{
    ...
    if (me->getVictim())
    {
        sLog->outError("PlayerbotAI: %s tried following while still fighting!", me->GetName());
        return;
    }
    ...
    {
        if (CanFollowOthers() && _followTargetGUID == 0)
            sLog->outError("PlayerbotAI: %s should follow someone else but has no target specified!", me->GetName());
        else if (!CanFollowOthers() && _followTargetGUID != 0)
            sLog->outError("PlayerbotAI: %s has follow target specified but cannot follow it!", me->GetName());
        //force follow master
        _followTargetGUID = 0;
    ...
    if (!fTarget)
    {
        sLog->outError("PlayerbotAI: %s has no follow target! Set to master...", me->GetName());
        fTarget = GetMaster();
        _followTargetGUID = 0;
        ClearMovementFlag(MOVEMENT_FLAG_TARGET_CHANGED);
    }
...
void PlayerbotAI::UpdateMountedState(uint32 const diff)
{
    ...
    if (!auraList.empty())
    {
        ...
        if (!spellInfo)
        {
            sLog->outError("PlayerbotAI: %s tried to mount but master %s is mounted by spell with no spellInfo!", me->GetName(), GetMaster()->GetName());
            return;
        }
        ...
        {
            sLog->outError("PlayerbotAI: %s cannot find a proper mount!", me->GetName());
...
Unit* PlayerbotAI::GetAttackTarget(bool /*ranged*/, bool byspell, bool &reset) const
{
    ...
    if (u && u == mytar)
    {
        sLog->outError("PlayerbotAI::GetAttackTarget(): bot %s continues attack common target %s", me->GetName(), u->GetName());
        return u;
    }
    ...
    //{
    //    //sLog->outError("bot %s cannot attack target %s, too far away", me->GetName(), mytar ? mytar->GetName() : "");
    //    return NULL;
    //}
    ...
    {
        sLog->outError("PlayerbotAI::GetAttackTarget(): bot %s starts attack master's target %s", me->GetName(), u->GetName());
        return u;
    }
    ...
    if (CanPlayerbotAttack(me, mytar, byspell)/* && !IsUnitInDuel(mytar, GetMaster())*/)
    {
        ...
        sLog->outError("PlayerbotAI::GetAttackTarget(): bot %s continues attack its target %s (reset = %u)", me->GetName(), mytar->GetName(), uint8(reset));
        return mytar;
    }
    ...
    {
		Group* _group;
        for (GroupReference* ref = _group->GetFirstMember(); ref != NULL; ref = ref->next())
        {
    ...
    for (std::set<Player*>::const_iterator itr = playerSet.begin(); itr != playerSet.end(); ++itr)
    {
        ...
        {
            sLog->outError("PlayerbotAI::GetAttackTarget(): bot %s hooked %s's victim %s", me->GetName(), pl->GetName(), u->GetName());
            return u;
        }
        ...
        for (uint8 i = 0; i != pl->GetMaxNpcBots(); ++i)
        {
            ...
            {
                sLog->outError("PlayerbotAI::GetAttackTarget(): bot %s hooked %s's victim %s", me->GetName(), bot->GetName(), u->GetName());
                return u;
            }
            ...
            {
                sLog->outError("PlayerbotAI::GetAttackTarget(): bot %s hooked %s's victim %s", me->GetName(), pet->GetName(), u->GetName());
                return u;
            }
        }
    }
    ...
    if (t)
    {
        ...
        {
            sLog->outError("PlayerbotAI::GetAttackTarget(): bot %s has Found a new target %s", me->GetName(), t->GetName());
            reset = true;
        }
    }
...
uint32 PlayerbotAI::InitSpell(Player* caster, uint32 baseSpell)
{
    ...
    //sLog->outError("PlayerbotAI::InitSpell() proceed spell %u (%s): forwarding to %u (%s)", baseSpell, spellInfo->SpellName[0], next, sSpellMgr->GetSpellInfo(next)->SpellName[0]);
...
bool PlayerbotAI::CastSpell(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    {
        sLog->outError("PlayerbotAI: %s trying to cast wrong spell on unit! %s (%u) on %s", me->GetName(), spellInfo->SpellName[uint8(GetMaster()->GetSession()->GetSessionDbcLocale())], spellInfo->Id, victim->GetName());
        //return false;
    }

    sLog->outError("PlayerbotAI: %s casts: %s (%u) on %s", me->GetName(), spellInfo->SpellName[uint8(GetMaster()->GetSession()->GetSessionDbcLocale())], spellInfo->Id, victim->GetName());
...
bool PlayerbotAI::CastSpell(Item* item, SpellInfo const* spellInfo)
{
    ...
    {
        //sLog->outError("PlayerbotAI: %s trying to cast wrong spell on item! %s (%u) on %s", me->GetName(), spellInfo->SpellName[uint8(GetMaster()->GetSession()->GetSessionDbcLocale())], spellInfo->Id, item->GetTemplate()->Name1);
        return false;
    }

    //sLog->outError("PlayerbotAI: %s casts: %s (%u) on %s", me->GetName(), spellInfo->SpellName[uint8(GetMaster()->GetSession()->GetSessionDbcLocale())], spellInfo->Id, item->GetTemplate()->Name1);
...
void PlayerbotAI::_SendDebugInfo()
{
    ...
    ch.PSendSysMessage("Listing stats for %s:", me->GetName());
