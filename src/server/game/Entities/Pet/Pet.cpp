        ...
        case 28017: // Bloodworms
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - 30 - (petlevel / 4)) * sWorld->getRate(RATE_PET_ATT));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel - 30 + (petlevel / 4)) * sWorld->getRate(RATE_PET_ATT));
                }
        case HUNTER_PET:
        {
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case 1964: //force of nature
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel * 2.5f - (petlevel / 2) + bonusDmg) * sWorld->getRate(RATE_PET_ATT)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel * 2.5f + (petlevel / 2) + bonusDmg) * sWorld->getRate(RATE_PET_ATT)));
                    break;
                }
                ...
                case 15438: //fire elemental
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel * 4 - petlevel) * sWorld->getRate(RATE_PET_ATT)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel * 4 + petlevel) * sWorld->getRate(RATE_PET_ATT)));
                    break;
                }
                case 19668: // Shadowfiend
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(((petlevel * 4 - petlevel) + bonus_dmg) * sWorld->getRate(RATE_PET_ATT)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(((petlevel * 4 + petlevel) + bonus_dmg) * sWorld->getRate(RATE_PET_ATT)));

                    break;
                }
                case 19833: //Snake Trap - Venomous Snake
                {
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(((petlevel / 2) - 25) * sWorld->getRate(RATE_PET_ATT)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(((petlevel / 2) - 18) * sWorld->getRate(RATE_PET_ATT)));
                    break;
                }
                case 19921: //Snake Trap - Viper
                {
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel / 2 - 10) * sWorld->getRate(RATE_PET_ATT)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel / 2) * sWorld->getRate(RATE_PET_ATT)));
                    break;
                }
                case 29264: // Feral Spirit
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(((petlevel * 4 - petlevel) + (m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * dmg_multiplier * 2 / 14)) * sWorld->getRate(RATE_PET_ATT)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(((petlevel * 4 + petlevel) + (m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * dmg_multiplier * 2 / 14)) * sWorld->getRate(RATE_PET_ATT)));
...
{
    uint8 points = (level >= 10) ? ((level - 6) / 2) : 0;
