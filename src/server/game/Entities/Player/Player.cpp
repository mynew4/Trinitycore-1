    ...
    if (_group)
    {
        ...
        if (_maxNotGrayMember && player->isAlive() &&
            ...
            //设置组队瓜分经验
                ...
                uint32(xp * rate);      // Reward only HALF of XP if some of group members are gray.
        else
            xp = 0;
    }
    if (xp)
...
void KillRewarder::_RewardPlayer(Player* player, bool isDungeon)
{
    ...
    if (!_isPvP || _isBattleGround)
    {
        ...
        if (!_isBattleGround)
        {
            ...
            _RewardReputation(player, (isDungeon ? 1.0f : rate) * 2);
...
void Player::Update(uint32 p_time)
{
    ...
    if (HasAura(91109) && GetMapId() != 0)
    {
    RemoveAurasDueToSpell(91109);
    }
    
    if (HasAura(91110) && GetMapId() != 1)
    {
    RemoveAurasDueToSpell(91110);
    }
...
void Player::GiveXP(uint32 xp, Unit* victim, float group_rate)
{
    ...
    if (level >= 60 && !GetAchievementMgr().HasAchieved(42) || level >= 60 && !GetAchievementMgr().HasAchieved(43)) {
        ModifyMoney(xp * 0.5);
        return;
    } else if (level >= 70 && !GetAchievementMgr().HasAchieved(1287) || level >= 70 && !GetAchievementMgr().HasAchieved(44)) {
    ModifyMoney(xp * 0.5);
        return;
    ...
    if (!(currFields & val))
    {
        ...
        if (areaEntry->area_level > 0)
        {
            ...
            else
            {
                ...
                if (HasAura(91109)) 
                {
                XP = XP * sWorld->getRate(BUFF_RATE_XP_EXPLORE);
                }
                if (HasAura(91110)) 
                {
                XP = XP * sWorld->getRate(BUFF_RATE_XP_EXPLORE);
...
void Player::CreateBot(uint32 botentry, uint8 botrace, uint8 botclass, bool istank, bool revive)
{
    ...
    {
        sLog->outError("ERROR! CreateBot(): player %s (%u) trying to create bot with entry = %u, race = %u, class = %u, ignored", GetName(), GetGUIDLow(), botentry, botrace, botclass);
        for (uint8 pos = 0; pos != GetMaxNpcBots(); ++pos)
            if (m_botmap[pos]->m_entry == botentry)
                ClearBotMustBeCreated(pos, false);
        return;
    ...
    if (!_set)
    {
        sLog->outError("character %s (%u) is failed to create npcbot! Removing all bots", GetName(), GetGUIDLow());
...
void Player::CreateNPCBot(uint8 bot_class)
{
        ...
        bot_class != CLASS_ROGUE))
    {
        ...
        sLog->outError("Player::CreateNPCBot(): Character %u tried to create npcbot of class %s, which is not allowed on your server!", GetGUIDLow(), bclass);
