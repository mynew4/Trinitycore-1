...
void WorldSession::HandlePlayerBotLogin(LoginQueryHolder* holder)
{
    ...
    if (!mSession)
    {
        //sLog->outError("Failed to create bot. Session if not found!");
        delete holder;
        return;
    }
    ...
    if (!master)
    {
        //sLog->outError("Failed to create bot. Master if not found!");
        delete holder;
        return;
    }
    //sLog->outError("Creating bot for %s...", master->GetName().c_str());
