    ...
    struct boss_black_knightAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* pKiller)
        {
            ...
            if (pInstance)
            {
                ...
                if (IsHeroic())
                {
                    AchievementEntry const* AchievShatterResistant = GetAchievementStore()->LookupEntry(ACHIEV_WORSE);
                    AchievementEntry const* AchievShatterResistant2 = GetAchievementStore()->LookupEntry(ACHIEV_HEROIC_DONE_A);
                    AchievementEntry const* AchievShatterResistant3 = GetAchievementStore()->LookupEntry(ACHIEV_HEROIC_DONE_H);
                    Map* map = me->GetMap();
                    ...
                    {
                        itr->getSource()->CompletedAchievement(AchievShatterResistant2);
                    }else
                    {
                        itr->getSource()->CompletedAchievement(AchievShatterResistant3);
                    }
                  }
}
                } else {
                AchievementEntry const* AchievShatterResistant4 = GetAchievementStore()->LookupEntry(4296);
                Map* map = me->GetMap();
                    if (map && map->IsDungeon()) {
                    Map::PlayerList const &players = map->GetPlayers();
                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr) {
                    itr->getSource()->CompletedAchievement(AchievShatterResistant4);
}
}
}
            }
