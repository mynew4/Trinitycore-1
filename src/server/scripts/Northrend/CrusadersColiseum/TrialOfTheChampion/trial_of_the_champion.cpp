...
#define GOSSIP_START_EVENT1     "Sono pronto per iniziare la sfida."
#define GOSSIP_START_EVENT2     "Sono pronto per una nuova sfida."
    ...
    struct npc_announcer_toc5AI : public ScriptedAI
    {
        ...
        void SetData(uint32 uiType, uint32 uiData)
        {
			...
			PERSONALIZZAZIONE TALECRAFT
	            */
		    //forzo l'attacco dei champion in maniera tale da eludere il bug della mancanza dei soldati a cavallo
                ...
                case DATA_LESSER_CHAMPIONS_DEFEATED:
                {
                    ...
                    if (uiLesserChampions == 3 || uiLesserChampions == 6)
                    {
                        ...
                        for (std::list<uint64>::const_iterator itr = TempList.begin(); itr != TempList.end(); ++itr)
                            	//rimuovo il controllo del tipo di creature presente sperando che in questo modo funzioni l'aggro dei boss prima dell'uccisione dei trash
